...
```bash
pip install kafka-python-ng  # Required for Kafka producer communication
...
```bash
docker run --rm --name my-api-ingest -p 80:80 api-ingest
```
...
- The `kafka-python-ng` package is required as the `main.py` script contains methods to communicate with a Kafka producer.
